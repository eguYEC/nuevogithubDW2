...
require_once "data.inc"; // Asegúrate de que este archivo defina $listainventario

$title = "Editar Inventario"; // Título cambiado

// recuperar el parametro GET
$idInventario = base64_decode($_GET["idInventario"]); // Nombre de variable y parámetro GET cambiados

if ( isset($_SESSION["sInventario"]) ){ // Usar la sesión sInventario
    $listaInventarioTemp = unserialize($_SESSION["sInventario"]);
}else{
    $listaInventarioTemp = $listainventario; // Usar listainventario
}
...
foreach($listaInventarioTemp as $item){ // Iterar a través de los elementos del inventario
...
// Ya no se necesita código relacionado con la liga para la edición de inventario
// $listaLigasTemp y $opciones se eliminan.
...
<div class='ctn-main'>
    <div class='ctn-form'>
        <div class='form-header'>
            ...
            <a href='inventario_list.php'><div class='btn-back'></div></a> </div>
        <div class='form-content'>
            <form action='inventario_update.php' method='POST'> <input type='hidden' name='idInventario' value='<?php echo htmlspecialchars($idInventario) ?>'>
                <div class='x-1'>
                    <div class='ctn-control'>
                        <input type='text' name='producto' placeholder='Ingrese el nombre del producto' value='<?php echo htmlspecialchars($producto) ?>'>
                    </div>
                </div>
                <div class='x-1'>
                    <div class='ctn-control'>
                        <input type='text' name='precio' placeholder='Ingrese el precio' value='<?php echo htmlspecialchars($precio) ?>'>
                    </div>
                </div>
                <div class='x-1'>
                    <div class='ctn-control'>
                        <input type='text' name='cantidad' placeholder='Ingrese la cantidad' value='<?php echo htmlspecialchars($cantidad) ?>'>
                    </div>
                </div>
                <div class='x-1'>
                    <div class='ctn-control'>
